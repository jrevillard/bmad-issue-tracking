...
- **CI architecture restructured**: `ci-wait.sh` (script shell qui attendait la CI) remplacé par `ci-status.sh` (script shell qui lit `ci-status.json`) + `story-track` workflow LLM (qui attend la CI et écrit le diagnostic). L'intelligence (poll CI, parse logs, retry flaky) est maintenant dans le workflow LLM, le verify command est déterministe et rapide.
...
- `ci-wait` bmad-loop `[verify]` command (`assets/bmad-loop/ci-gate/ci-wait.sh`, deployed by setup step 3c): pushes the story branch (propagating the current code, including deterministic repairs) and waits for its remote pipeline to go green. A red CI fails the verify command, which bmad-loop answers with a feedback-driven repair session (re-running `bmad-build-auto` with the failing CI output) — the story is auto-fixed and re-verified, bounded by `max_dev_attempts`, before the merge-back.
- `story-track` bmad-loop plugin (`assets/bmad-loop/story-track/`, deployed by setup step 3c): an LLM workflow session at `post_review_result` that pushes the final story branch, ensures the trace MR exists, and mirrors each story to its GitLab/GitHub issue (status label, result comment, MR link) — per-story tracking during the run, before the CI check.
