...
- `assets/bmad-loop/ci-gate/ci-status.sh` — bmad-loop `[verify]` command deployed to `.bmad-loop/ci-status.sh` (setup step 3c): reads `ci-status.json` (written by story-track) and returns exit 0 if CI is green, exit 1 if red. The intelligent work (polling CI, parsing logs) is done by story-track (LLM workflow).
- `assets/bmad-loop/story-track/` — bmad-loop plugin (deployed to `.bmad-loop/plugins/story-track/`, setup step 3c) declaring a `[workflows.story-track]` LLM session at `post_review_result`, `blocking=true`: pushes the final story branch, ensures the trace MR exists, waits for CI to complete, writes `ci-status.json` with the result (green or red with diagnostic), and mirrors the story to its GitLab/GitHub issue. Runs BEFORE the `ci-status.sh` verify, so the CI result is captured when the check runs. The trace MR is left open (GitLab auto-marks it merged after the local merge-back is pushed).
