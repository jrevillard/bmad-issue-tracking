...
- `assets/bmad-loop/ci-gate/ci-status.sh` — bmad-loop `[verify]` command deployed to `.bmad-loop/ci-status.sh` (setup step 3c): reads `ci-status.json` (written by story-track-dev or story-track-review) and returns exit 0 if CI is green, exit 1 if red. The intelligent work (polling CI, parsing logs) is done by the LLM workflows.
- `assets/bmad-loop/story-track-dev/` — bmad-loop plugin (deployed to `.bmad-loop/plugins/story-track-dev/`, setup step 3c) declaring a `[workflows.story-track-dev]` LLM session at `post_dev_phase`, `blocking=true`: pushes the code dev, waits for CI, writes `ci-status.json`, and creates the trace MR. Runs for EVERY story that completes dev, regardless of whether review happens afterward.
- `assets/bmad-loop/story-track-review/` — bmad-loop plugin (deployed to `.bmad-loop/plugins/story-track-review/`, setup step 3c) declaring a `[workflows.story-track-review]` LLM session at `post_review_result`, `blocking=false`: commits review modifications, pushes, waits for CI, writes `ci-status.json`, and mirrors the story to its GitLab/GitHub issue. Runs ONLY when review completes. Non-blocking: issue tracking is best-effort.
